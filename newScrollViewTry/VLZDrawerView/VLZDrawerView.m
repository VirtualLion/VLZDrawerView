...
@interface VLZDrawerView ()<UIScrollViewDelegate>
...
- (UIScrollView *)scrollView{
    ...
    if (self.scrollViews.count == 0) {
        ...
        for (VLZDrawerModel * imageModel in _data) {
            
            VLZDrawerImageView * imageView = [VLZDrawerImageView new];
...
+ (instancetype)newWithFrame:(CGRect)frame
    ...
    VLZDrawerView * view = [VLZDrawerView new];
    view.frame = frame;
...
- (void)onTap:(UITapGestureRecognizer *)sender{
    ...
    for (VLZDrawerImageView * imageView in [self.scrollViews.firstObject subviews].reverseObjectEnumerator) {
        
        CGRect rect = [imageView convertRect:imageView.bounds toView:self];
        if (CGRectContainsPoint(rect, tapPoint)) {
            
            if ([imageView isKindOfClass:[VLZDrawerImageView class]]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (offsetY >= 0) {
        ...
        for (NSInteger i = 0; i < _data.count; i++) {
            VLZDrawerImageView * imageView = [self.scrollViews.firstObject viewWithTag:i+100];
